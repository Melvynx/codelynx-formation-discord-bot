 ...
 */
export const getChannelsByCategoryIdAsync = async(client: ArcClient, categoryId: string) => {
 ...
 */
export const getTicketsChannels = async(client: ArcClient) => {
  const createdCategory = await client.channels.fetch(env.CREATE_TICKET_CATEGORY_ID);
  if (!createdCategory || createdCategory.type != ChannelType.GuildCategory) return null;

  const claimedCategory = await client.channels.fetch(env.CLAIMED_TICKET_CATEGORY_ID);
  if (!claimedCategory || claimedCategory.type != ChannelType.GuildCategory) return null;

  const closedCategory = await client.channels.fetch(env.CLOSED_TICKET_CATEGORY_ID);
  if (!closedCategory || closedCategory.type != ChannelType.GuildCategory) return null;
